...
public class TrackOrderPage {
	...
	@FindBy(xpath="//form[@action='/account/trackorder']")
	WebElement trackOrderFrame;
	
	@FindBy(id="email")
	WebElement emailIDBox;
	
	@FindBy(id="partialOrderId")
	...
	public WebDriver checkIfTrackOrderFormExists(WebDriver driver)
	{
		...
		Wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(strTrackOrderFrameXpath)));
		
		//driver.switchTo().frame(strTrackOrderFrameXpath);
		
		if(driver.findElements(By.xpath(strTrackOrderFrameXpath)).size() == 0)
		{
